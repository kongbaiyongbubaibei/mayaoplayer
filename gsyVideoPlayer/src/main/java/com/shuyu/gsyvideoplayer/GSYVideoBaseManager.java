...
//                    setNeedMute(false);
    ...
    public void setNeedMute(boolean needMute) {
        ...
        Log.i(TAG, "setNeedMute: " + needMute);
        if (playerManager != null) {
            playerManager.setNeedMute(needMute);
        }
        if (listener() != null) {
            listener().onNeedMute(needMute);
        }
    }
     ...
     * 是否需要静音
     */
    public void setVolume(boolean needMute) {
        this.needMute = needMute;
        Log.i(TAG, "setNeedMute: " + needMute);
        if (playerManager != null && needMute) {
            playerManager.setVolume(0, 0);
        }
        if (listener() != null) {
            listener().onNeedMute(needMute);
        }
